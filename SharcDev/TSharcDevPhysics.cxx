...
TSharcDevPhysics::TSharcDevPhysics(){
	m_Rand= new TRandom3();
	EventMultiplicity   = 0 ;
	m_EventData         = new TSharcDevData ;
	m_PreTreatedData    = new TSharcDevData ;
...
void TSharcDevPhysics::BuildPhysicalEvent(){
    ...
    // Fall Back option, take the Strip Calibration
      ...
      name = "SHARCDEV/D"+ NPL::itoa(N)+"_STRIP_FRONT"+ NPL::itoa(Front)+"_DEADLAYER";
      DeadLayer.push_back(Cal->GetValue(name,0));
    }
    else{
      Strip_E.push_back(Back_E) ;
      name = "SHARCDEV/D"+ NPL::itoa(N)+"_STRIP_BACK"+ NPL::itoa(Back)+"_DEADLAYER";
      DeadLayer.push_back(Cal->GetValue(name,0));
    }
...
////////////////////////////////////////////////////////////////////////////
bool TSharcDevPhysics :: IsValidChannel(const string& DetectorType, const int& telescope , const int& channel){
...
///////////////////////////////////////////////////////////////////////////
void TSharcDevPhysics::ReadAnalysisConfig(){
  bool ReadingStatus = false;

  // path to file
  string FileName = "./configs/ConfigSharcDev.dat";
  ...
  if (!AnalysisConfigFile.is_open()) {
    cout << " No ConfigSharcDev.dat found: Default parameter loaded for Analayis " << FileName << endl;
    return;
  }
  cout << " Loading user parameter for Analysis from ConfigSharcDev.dat " << endl;
  ...
  while (!AnalysisConfigFile.eof()) {
    ...
    // search for "header"
    if (LineBuffer.compare(0, 11, "ConfigSharcDev") == 0) ReadingStatus = true;
...
void TSharcDevPhysics::AddParameterToCalibrationManager(){
  ...
  for(int i = 0 ; i < m_NumberOfDetector ; ++i){
    ...
    for( int j = 0 ; j < 24 ; ++j){
      ...
      Cal->AddParameter("SHARCDEV", "D"+ NPL::itoa(i+1)+"_STRIP_FRONT"+ NPL::itoa(j+1)+"_E","SHARCDEV_D"+ NPL::itoa(i+1)+"_STRIP_FRONT"+ NPL::itoa(j+1)+"_E")   ;
      Cal->AddParameter("SHARCDEV", "D"+ NPL::itoa(i+1)+"_STRIP_FRONT"+ NPL::itoa(j+1)+"_DEADLAYER",
                        "SHARCDEV_D"+ NPL::itoa(i+1)+"_STRIP_FRONT"+ NPL::itoa(j+1)+"_DEADLAYER")   ;
      Cal->AddParameter("SHARCDEV", "D"+ NPL::itoa(i+1)+"_STRIP_FRONT"+ NPL::itoa(j+1)+"_T","SHARCDEV_D"+ NPL::itoa(i+1)+"_STRIP_FRONT"+ NPL::itoa(j+1)+"_T")   ;

      // Pixel Calibration
      for( int k = 0 ; k < 48 ; ++k){
        Cal->AddParameter("SHARCDEV", "D"+ NPL::itoa(i+1)+"_STRIP_FRONT"+ NPL::itoa(j+1)+"_BACK"+ NPL::itoa(k+1)+"_E","SHARCDEV_D"+ NPL::itoa(i+1)+"_STRIP_FRONT"+ NPL::itoa(j+1)+"_BACK"+ NPL::itoa(k+1)+"_E")   ;
        Cal->AddParameter("SHARCDEV", "D"+ NPL::itoa(i+1)+"_STRIP_FRONT"+ NPL::itoa(j+1)+"_BACK"+ NPL::itoa(k+1)+"_DEADLAYER",
                          "SHARCDEV"+ NPL::itoa(i+1)+"_STRIP_FRONT"+ NPL::itoa(j+1)+"_BACK"+ NPL::itoa(k+1)+"_DEADLAYER")   ;
    ...
    for( int j = 0 ; j < 48 ; ++j){
      ...
      Cal->AddParameter("SHARCDEV", "D"+ NPL::itoa(i+1)+"_STRIP_BACK"+ NPL::itoa(j+1)+"_E","SHARCDEV_D"+ NPL::itoa(i+1)+"_STRIP_BACK"+ NPL::itoa(j+1)+"_E")   ;
      Cal->AddParameter("SHARCDEV", "D"+ NPL::itoa(i+1)+"_STRIP_BACK"+ NPL::itoa(j+1)+"_DEADLAYER",
                        "SHARCDEV_D"+ NPL::itoa(i+1)+"_STRIP_BACK"+ NPL::itoa(j+1)+"_DEADLAYER")   ;

      Cal->AddParameter("SHARCDEV", "D"+ NPL::itoa(i+1)+"_STRIP_BACK"+ NPL::itoa(j+1)+"_T","SHARCDEV_D"+ NPL::itoa(i+1)+"_STRIP_BACK"+ NPL::itoa(j+1)+"_T")   ;
    }

    for( int j = 0 ; j < 1 ; ++j){
      // Pad Calibration
      Cal->AddParameter("SHARCDEV", "D"+ NPL::itoa(i+1)+"_PAD_E","SHARCDEV_D"+ NPL::itoa(i+1)+"_PAD_E")   ;
      Cal->AddParameter("SHARCDEV", "D"+ NPL::itoa(i+1)+"_PAD_T","SHARCDEV_D"+ NPL::itoa(i+1)+"_PAD_T")   ;
    }
  }
...
void TSharcDevPhysics::InitializeRootInputRaw(){
  ...
  if(inputChain->FindBranch("fSharcDev_*"))
    inputChain->SetBranchStatus( "fSharcDev_*",true);
  inputChain->SetBranchAddress( "SharcDev" , &m_EventData );
...
namespace SharcDev_LOCAL{
  //   DSSD
  //   Front
  double fStrip_Front_E(const TSharcDevData* m_EventData , const int& i){
    static CalibrationManager* Cal = CalibrationManager::getInstance();
    static string name ;
    name = "SHARCDEV/D" + NPL::itoa( m_EventData->GetFront_DetectorNbr(i) ) + "_STRIP_FRONT" + NPL::itoa( m_EventData->GetFront_StripNbr(i) ) + "_E";
    return Cal->ApplyCalibration(name,m_EventData->GetFront_Energy(i) );
  }

  double fStrip_Front_T(const TSharcDevData* m_EventData , const int& i){
    static CalibrationManager* Cal = CalibrationManager::getInstance();
    static string name ;
    name ="SHARCDEV/D" + NPL::itoa( m_EventData->GetFront_DetectorNbr(i) ) + "_STRIP_FRONT" + NPL::itoa( m_EventData->GetFront_StripNbr(i) ) +"_T"; 

    return Cal->ApplyCalibration(name, m_EventData->GetFront_TimeCFD(i) );
  }

  //   Back
  double fStrip_Back_E(const TSharcDevData* m_EventData , const int& i){
    static CalibrationManager* Cal = CalibrationManager::getInstance();
    static string name ;
    name =  "SHARCDEV/D" + NPL::itoa( m_EventData->GetBack_DetectorNbr(i) ) + "_STRIP_BACK" + NPL::itoa( m_EventData->GetBack_StripNbr(i)) +"_E";

    return Cal->ApplyCalibration(name, m_EventData->GetBack_Energy(i) );
  }

  double fStrip_Back_T(const TSharcDevData* m_EventData , const int& i){
    static CalibrationManager* Cal = CalibrationManager::getInstance();
    static string name ;
    name = "SHARCDEV/D" + NPL::itoa( m_EventData->GetBack_DetectorNbr(i) ) + "_STRIP_BACK" + NPL::itoa( m_EventData->GetBack_StripNbr(i) ) +"_T"; 

    return Cal->ApplyCalibration(name, m_EventData->GetFront_TimeCFD(i));
  }

  //   PAD
  double fPAD_E(const TSharcDevData* m_EventData , const int& i){
    static CalibrationManager* Cal = CalibrationManager::getInstance();
    static string name ;
    name = "SHARCDEV/D" + NPL::itoa( m_EventData->GetPAD_DetectorNbr(i) ) + "_PAD_E";

    return Cal->ApplyCalibration(name,m_EventData->GetPAD_Energy(i) );
  }

  double fPAD_T(const TSharcDevData* m_EventData , const int& i){
    static CalibrationManager* Cal = CalibrationManager::getInstance();
    static string name ;
    name = "SHARCDEV/D" + NPL::itoa( m_EventData->GetPAD_DetectorNbr(i) ) + "_PAD_T";
...
class proxy_sharcdev{
  public:
    proxy_sharcdev(){
      NPL::DetectorFactory::getInstance()->AddToken("SharcDev","SharcDev");
      NPL::DetectorFactory::getInstance()->AddDetector("SharcDev",TSharcDevPhysics::Construct);
    }
};

proxy_sharcdev p_sharcdev;
}
